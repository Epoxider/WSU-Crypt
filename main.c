...
unsigned short G_Encrypt(unsigned short w, unsigned long round, unsigned long * keyPtr) {
    unsigned char g1 = (w >> 8) &0xff;
    unsigned char g2 = w & 0xff;
    unsigned char g3, g4, g5, g6;

    g3 = FTable(g2^K_Encrypt(4*round, keyPtr))^g1;
    g4 = FTable(g3^K_Encrypt(4*round + 1, keyPtr))^g2;
    g5 = FTable(g4^K_Encrypt(4*round + 2, keyPtr))^g3;
    g6 = FTable(g5^K_Encrypt(4*round + 3, keyPtr))^g4;

    return ((unsigned short) (g5)) << 8 | (unsigned short) g6;

}

void funcF(unsigned short r0, unsigned short r1, unsigned long round) {

    unsigned short t0 = G(r0, round);
    unsigned short t1 = G(r1, round);
