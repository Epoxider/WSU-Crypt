...
struct Fvalues funcF(unsigned short r0, unsigned short r1, unsigned long long round, unsigned long long * keyPtr) {
...
unsigned long long encrypt(unsigned char * block, unsigned long long * keyPtr) {
    ...
    for(i = 0 ; i < 16; i++) {
        ...
        fvals = funcF(r0, r1, round, keyPtr);
...
int main(int argc, char* argv[]) {
    ...
    while (readSize > 0) {
        ...
        for (i = 0; i < 8; i++){
            sscanf(&blockAscii[i * 2], "%2hhx", &(block[i]));
        }

        if(isEndcode) {
            unsigned long long encryptBlock = encrypt(block, &key);
            fprintf(outputFilePtr, "%llx", encryptBlock);
        } else {
            //unsigned long long decryptBlock = decrypt(block, &key);
